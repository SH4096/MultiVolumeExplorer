...
class MultiVolumeIntensityChartView:
  ...
  def bgMultiVolumeNode(self, bgMultiVolumeNode):
    ...
    if self.__chartTableNode:
      cjyx.drmlScene.RemoveNode(self.__chartTableNode)
  ...
  def __init__(self):
    self.__chartView = cjyx.qDRMLPlotView()
    self.__chartView.setDRMLScene(cjyx.drmlScene)
    self.__chartViewNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLPlotViewNode")
    self.__chartView.setDRMLPlotViewNode(self.__chartViewNode)
    self.__chart = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLPlotChartNode")
  ...
  def clearPlots(self):
    ...
    if self.__bgPlot:
      cjyx.drmlScene.RemoveNode(self.__bgPlot)
    if self.__fgPlot:
      cjyx.drmlScene.RemoveNode(self.__fgPlot)

  def createNewVTKTableNode(self, xArray, yArray):
    chartTableNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLTableNode", "bgChartTable")
  ...
  def createChart(self, sliceWidget, xy, ignoreCurrentBackground=False):
    ...
    if self.__fgMultiVolumeNode:
      ...
      if self.__fgChartTableNode:
        cjyx.drmlScene.RemoveNode(self.__fgChartTableNode)
      self.__fgChartTableNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLTableNode", "fgChartTable")
    ...
    self.__bgPlot = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLPlotSeriesNode", "1st multivolume")
    ...
    self.__bgPlot.SetPlotType(cjyx.vtkDRMLPlotSeriesNode.PlotTypeScatter)
    if useFg:
      self.__bgPlot.SetLineStyle(cjyx.vtkDRMLPlotSeriesNode.LineStyleNone)
    else:
      self.__bgPlot.SetMarkerStyle(cjyx.vtkDRMLPlotSeriesNode.MarkerStyleNone)
    self.__chart.AddAndObservePlotSeriesNodeID(self.__bgPlot.GetID())
    # bgPlot.SetLabel("Primary multivolume ")

    if useFg:
      self.__fgPlot = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLPlotSeriesNode", "2nd multivolume")
      ...
      self.__fgPlot.SetPlotType(cjyx.vtkDRMLPlotSeriesNode.PlotTypeScatter)
      self.__fgPlot.SetColor(1,0,0)
      self.__fgPlot.SetMarkerStyle(cjyx.vtkDRMLPlotSeriesNode.MarkerStyleNone)
...
class LabeledImageChartView:
  ...
  def calculateLabeledVoxelsMeanAndInitiateChartArray(self):
    ...
    for k in self.labeledVoxels.keys():
      self.dataNodes[k] = cjyx.drmlScene.AddNode(cjyx.vtkDRMLDoubleArrayNode())
  ...
  def createChartNodeAndInsertData(self):
    ...
    chartNode = cjyx.drmlScene.AddNode(cjyx.vtkDRMLChartNode())
  ...
  def initiateChartViewNode(self, chartNode):
    chartViewNodes = cjyx.drmlScene.GetNodesByClass('vtkDRMLChartViewNode')
