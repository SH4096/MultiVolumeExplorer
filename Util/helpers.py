...
def makeSubtract(dceName,f0,f1,outputName):
  ...
  dce = cjyx.util.getNode(dceName)
  output = cjyx.util.getNode(outputName)
...
def explodeMultivolume(mvName, refName):
  mv = cjyx.util.getNode(mvName)
  ref = cjyx.util.getNode(refName)
  mvi = mv.GetImageData()
  ...
  for i in range(mv.GetNumberOfFrames()):
    ...
    # clone reference
    frame = cjyx.modules.volumes.logic().CloneVolume(cjyx.drmlScene, ref,'frame'+str(i))
