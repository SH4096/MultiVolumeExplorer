...
import argparse, sys, shutil, os, cjyx
...
def getMultivolumeFrame(mv, ref, frameNumber=0):
  ...
  # clone reference
  frame = cjyx.modules.volumes.logic().CloneVolume(cjyx.drmlScene, ref,'frame')
...
def main(argv):
  if True:
    ...
    (_,mv) = cjyx.util.loadVolume(args.multivolume,returnNode=True)
    (_,label) = cjyx.util.loadLabelVolume(args.label,returnNode=True)
